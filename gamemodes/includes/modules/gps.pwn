CMD:map(playerid)
{
	...
	Dialog_Show(playerid, DIALOG_GPS, DIALOG_STYLE_LIST, "{3300FF}HE THONG BAN DO{FFFFFF}", "Tim Viec Lam {FFFF33}(VCITY-MU)\n{FFFFFF}Tim Duong {FFFF33}(VCITY-MU){FFFFFF}", "Select", "Close");
	return 1;
}
...
Dialog:DIALOG_GPS(playerid, response, listitem, inputtext[])
{
	if (response)
	{
		switch(listitem)
		{
			case 0:
			{
				Dialog_Show(playerid, DIALOG_TIMVIEC, DIALOG_STYLE_LIST, "{3300FF}VI TRI CONG VIEC{FFFFFF}", "Giao Xi Mang {FFFF33}(LS){FFFFFF}\nPizza Boy {FFFF33}(LS){FFFFFF}\nKhai Thac Da {FFFF33}(SF){FFFFFF}\nVe Si {FFFF33}(LS){FFFFFF}\nNguoi Ban Vu Khi {FFFF33}(LS){FFFFFF}\nTai Xe Taxi {FFFF33}(LS){FFFFFF}\nTho Thu Cong {FFFF33}(LS){FFFFFF}", "Select", "Close");
			}
			case 1:
			{
				Dialog_Show(playerid, DIALOG_TIMDUONG, DIALOG_STYLE_LIST, "{3300FF}VI TRI DIA DIEM{FFFFFF}", "Toa Thi Chinh {FFFF33}(LS)\n{FFFFFF}So Canh Sat {FFFF33}(LS){FFFFFF}\n{FFFFFF}Cua Hang 24/7 {FFFF33}(LS){FFFFFF}\n{FFFFFF}VIP {FFFF33}(LS){FFFFFF}\n{FFFFFF}Benh Vien {FFFF33}(LS){FFFFFF}\n{FFFFFF}Ngan Hang {FFFF33}(LS){FFFFFF}\n{FFFFFF}Cua Hang Quan Ao {FFFF33}(LS){FFFFFF}\n{FFFFFF}Cay Xang {FFFF33}(LS){FFFFFF}\n{FFFFFF}Lay Vat Lieu {FFFF33}(LS){FFFFFF}", "Select", "Close");
			}
		}
	}
	return 1;
...
Dialog:DIALOG_TIMDUONG(playerid, response, listitem, inputtext[])
{
	if (response)
	{
		switch(listitem)
		{
		    ...
		    case 8:
			{
				SetPlayerCheckpointEx(playerid, 2390.56,-2007.87,13.55, 4.0); // lay vl
				CP[playerid] = 20;
                return 1;
			}
		}
...
Dialog:DIALOG_TIMVIEC(playerid, response, listitem, inputtext[])
{
	if (response)
	{
		switch(listitem)
		{
			...
			case 3: // ve si ( ls )
            {
				SetPlayerCheckpointEx(playerid, 1099.74, -1504.69, 15.79, 4.0);
				CP[playerid] = 20;
				return 1;
			}
			case 4: // ban vu khi ( ls )
            {
				SetPlayerCheckpointEx(playerid, 1366.14, -1275.19, 13.54, 4.0);
				CP[playerid] = 20;
				return 1;
			}
			case 5: // tai xe taxi ( ls )
            {
				SetPlayerCheckpointEx(playerid, 1741.68, -1863.54, 13.57, 4.0);
				CP[playerid] = 20;
				return 1;
			}
			case 6: // tho thu cong ( ls )
            {
				SetPlayerCheckpointEx(playerid, 2195.91, -1973.05, 13.55, 4.0);
				CP[playerid] = 20;
				return 1;
			}
		}
